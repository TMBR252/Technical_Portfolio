                    ...
                    `}>
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-1.5 md:gap-2 pl-2 md:pl-4 z-10 overflow-hidden h-6">
                                ...
                                <div className="relative w-[140px] sm:w-[200px] md:w-[280px] h-full flex items-center">
                ...
                <AnimatePresence>
                    {isExpanded && (
                        ...
                        >
                            ...
                            <div className="flex-1 flex flex-col pt-[max(1.5rem,3vh)] pb-0 justify-between relative min-h-0">
                                {/* Top Marquee — full width */}
                                <div className="flex-shrink-0 w-full">
                                    <Marquee />
                                </div>

                                <div className="flex-1 flex flex-col justify-center w-full relative min-h-0 px-6 sm:px-10 lg:px-[6vw] pt-[3vh] pb-[2vh]">

                                    {/* Close Button */}
                                    <div className="absolute top-0 right-6 sm:right-10 lg:right-[6vw] z-[10001]">
                                        <motion.button
                                            onClick={closeExpanded}
                                            className="relative p-3 sm:p-4 flex items-center justify-center"
                                        ...
                                        >
                                            ...
                                            >
                                                <X className="w-6 h-6 sm:w-7 sm:h-7 text-white dark:text-black" strokeWidth={2.5} />
                                            </motion.div>
                                        </motion.button>
                                    </div>

                                    {/* Main Grid — Links | Socials | Time | Version */}
                                    <div className="w-full grid grid-cols-2 lg:grid-cols-4 gap-x-8 lg:gap-x-16 gap-y-8 lg:gap-y-10">
                                        ...
                                        <FooterColumn title={t('socials')}>
                                            <div className="flex flex-col gap-1 w-fit">
                                                <div
                                                    className="relative flex flex-wrap items-center gap-2 group w-fit"
                                                    ...
                                                    <AnimatePresence>
                                                        {isEmailHovered && (
                                                            <motion.div
                                                                ...
                                                                exit={{ opacity: 0, x: 5 }}
                                                                className="hidden lg:flex absolute left-full ml-2 whitespace-nowrap items-center gap-2 z-50"
                                                            >
                                                                <span className="text-sm font-medium text-zinc-400 dark:text-zinc-500 select-all">
                                                ...
                                                <span className="lg:hidden text-sm font-medium text-zinc-400 dark:text-zinc-500 break-all">
                                                    {portfolioData.personal.email}
                                                </span>
                                            </div>
                                        ...
                                        <FooterColumn title={t('localTime')}>
                                            <p className="text-zinc-900 dark:text-white text-sm md:text-base lg:text-lg font-medium tracking-tight">
                                                {localTime}
                                            </p>
                                            <a
                                                href={`https://www.google.com/maps/search/${encodeURIComponent(portfolioData.personal.location)}`}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="text-zinc-900 dark:text-white text-sm md:text-base lg:text-lg font-medium tracking-tight hover:text-zinc-500 dark:hover:text-zinc-400 transition-colors inline-block"
                                            >
                                ...
                                {/* Bottom Brand Name — full viewport width */}
                                <div className="mt-auto flex-shrink-0 relative w-full overflow-visible px-[1.5vw] pb-[max(0.5rem,1vh)]">
                                    <motion.h2
                                        initial={{ opacity: 0, y: "40%" }}
                                        animate={isAboutExpanded ? { opacity: 0, y: "30%" } : { opacity: 1, y: 0 }}
                                        transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
                                        className="w-full text-center font-black leading-[0.8] text-zinc-900 dark:text-white tracking-tighter select-none whitespace-nowrap text-[clamp(2.75rem,13.6vw,22rem)]"
                                    >
...
function FooterColumn({ title, children }: { title: string; children: React.ReactNode }) {
    return (
        <div className="flex flex-col gap-3 md:gap-4">
            <h3 className="text-zinc-500 text-xs md:text-sm font-bold tracking-widest uppercase">{title}</h3>
    ...
    return (
        ...
        >
            <Link
                ...
                className="text-zinc-900 dark:text-white text-sm md:text-base lg:text-lg font-medium hover:text-zinc-500 dark:hover:text-zinc-400 transition-colors w-fit block"
            >
                {children}
            </Link>
        </motion.div>
...
function AboutHoverMenu({ tNav, onExpandChange }: { tNav: (key: string) => string; onExpandChange: (expanded: boolean) => void }) {
    ...
    const subLinks = [
        ...
        ...(SITE_FEATURES.fullAboutNav ? [{ href: '/skills', label: tNav('skills') }] : []),
        ...(SITE_FEATURES.fullAboutNav
            ...
            : []),
        { href: portfolioData.personal.linktreeUrl || 'https://linktr.ee/', label: tNav('linktree'), external: true },
    ];
    ...
    return (
        ...
        >
            ...
            >
                <Link
                    href="#about"
                    className="text-zinc-900 dark:text-white text-sm md:text-base lg:text-lg font-medium group-hover:text-zinc-500 dark:group-hover:text-zinc-400 transition-colors"
                >
                    {tNav('about')}
                </Link>
                <motion.div
