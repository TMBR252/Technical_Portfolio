...
const TechCard = ({ tech, idx, isLowPowerMode }: { tech: TechItem, idx: number, isLowPowerMode?: boolean }) => {
    ...
    const description =
        tech.description || techDescriptions[tech.name] || `Used across the case studies.`;
            ...
            <div className="w-[60px] h-[60px] rounded-[14px] flex-shrink-0 flex items-center justify-center bg-gray-50 dark:bg-background relative overflow-hidden transition-all group-hover:bg-white dark:group-hover:bg-background/80 shadow-inner group-hover:shadow-md">
                <div className="w-8 h-8 relative">
                    <Image
                        ...
                        alt={tech.name}
                        fill
                        className={tech.invertDark
                            ? 'object-contain grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all duration-300 dark:brightness-0 dark:invert'
                            : 'object-contain grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all duration-300'}
                        unoptimized
                        loading="lazy"
                    />
                </div>
