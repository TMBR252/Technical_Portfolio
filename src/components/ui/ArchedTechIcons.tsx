...
import { motion, useMotionValue, useTransform, PanInfo, animate, type MotionValue } from 'framer-motion';
import { useIsMobile } from '@/hooks/useIsMobile';

export interface ArchIcon {
  src: string;
  invertDark?: boolean;
}
...
function WheelIcon({ icon, angle, rotation }: { icon: ArchIcon; angle: number; rotation: MotionValue<number> }) {
  // Cancel both the wheel's live rotation and this icon's static angle so the
  // artwork stays upright no matter where the drag leaves it.
  const upright = useTransform(rotation, (v) => -v - angle);
        ...
        >
          <div className="w-full h-full relative" style={{ WebkitBackfaceVisibility: 'hidden', backfaceVisibility: 'hidden' }}>
            <Image
              ...
              alt="tech-icon"
              fill
              className={icon.invertDark ? 'object-contain opacity-90 transform-gpu dark:brightness-0 dark:invert' : 'object-contain opacity-90 transform-gpu'}
              unoptimized
              priority={true}
              draggable={false}
            />
...
export function ArchedTechIconsInteractive({ icons }: ArchedIconsProps) {
  ...
  // Touch / narrow: static icon grid - no drag wheel or cursor-none
  if (isMobile) {
    return (
      <div className="relative w-full max-w-full z-0 py-6 px-4">
        <div className="grid grid-cols-4 sm:grid-cols-5 gap-3 place-items-center">
          {icons.slice(0, 12).map((icon, i) => (
            <div
              key={`static-${icon.src}-${i}`}
              className="relative w-14 h-14 rounded-xl bg-white/95 dark:bg-[#0a0a0a]/95 border border-gray-200 dark:border-white/10 flex items-center justify-center p-2.5 shadow-sm"
            >
              <div className="w-full h-full relative">
                <Image
                  ...
                  alt={`tech-icon-${i}`}
                  fill
                  className={icon.invertDark ? 'object-contain opacity-90 dark:brightness-0 dark:invert' : 'object-contain opacity-90'}
                  unoptimized
                  draggable={false}
                />
              </div>
