...
/** Hover / press / focus — brand green must fire on phone & tablet, not only mouse hover. */
const brandInteract =
    'group-hover:opacity-0 group-active:opacity-0 group-focus-within:opacity-0';
const brandInteractShow =
    'group-hover:opacity-[0.12] group-active:opacity-[0.12] group-focus-within:opacity-[0.12]';

function brandBorder(theme: string, isActive: boolean) {
    if (theme === 'dark') {
        return isActive
            ? 'border-brand shadow-[0_0_15px_rgb(var(--brand-rgb)/0.08)]'
            : 'border-white/10 hover:border-brand active:border-brand focus-visible:border-brand hover:shadow-[0_0_15px_rgb(var(--brand-rgb)/0.08)] active:shadow-[0_0_15px_rgb(var(--brand-rgb)/0.08)]';
    }
    return isActive
        ? 'border-brand shadow-md shadow-brand/10'
        : 'border-black/10 hover:border-brand active:border-brand focus-visible:border-brand hover:shadow-md hover:shadow-brand/10 active:shadow-md active:shadow-brand/10';
}

/** Scattered full grid cells that appear, then fade out; brand green on hover/press */
function GrowFadeGrid({ theme = 'dark' }: { theme?: string }) {
    ...
    return (
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
            {/* Grid lines: muted → brand green on hover / press / focus */}
            <div
                className={cn(
                    'absolute inset-0 opacity-[0.06] transition-opacity duration-500',
                    brandInteract
                )}
                style={{
                ...
                return (
                    <motion.div
                        ...
                        className={cn(
                            ...
                            isDark
                                ? 'bg-white/12 group-hover:bg-brand/25 group-active:bg-brand/25 group-focus-within:bg-brand/25'
                                : 'bg-black/8 group-hover:bg-brand/22 group-active:bg-brand/22 group-focus-within:bg-brand/22'
                        )}
                        style={{
                            left: col * gridSize,
                            top: row * gridSize,
...
function MegaBoxBig({ href, icon: Icon, title, desc, theme, pathname, stretch = false }: any) {
    const isActive = pathname === href || pathname?.startsWith(`${href}/`);
    
    return (
        <Link href={href} className={cn(
            "group relative flex flex-col justify-between rounded-2xl border p-5 transition-all duration-500 overflow-hidden touch-manipulation",
            "hover:scale-[1.02] hover:-translate-y-1 active:scale-[1.01] active:-translate-y-0.5",
            stretch ? "h-full min-h-[10.5rem] flex-1" : "h-40",
            theme === 'dark' ? "bg-[#161616]" : "bg-black/[0.02] hover:bg-white active:bg-white",
            brandBorder(theme, isActive)
        )}>
            <GrowFadeGrid theme={theme} />

            {/* Text stays black / grey (or white / grey in dark) for legibility */}
            <Icon className={cn(
                "w-6 h-6 relative z-10 transition-all duration-300 group-hover:scale-110 group-hover:text-brand group-active:scale-110 group-active:text-brand group-focus-within:text-brand",
...
function MegaBoxSmall({ href, icon: Icon, title, desc, theme, pathname, external }: any) {
    ...
    return (
        <Link
            ...
            className={cn(
            "group relative flex flex-col justify-center rounded-2xl border p-4 transition-all duration-500 overflow-hidden touch-manipulation",
            "hover:scale-[1.02] hover:-translate-y-1 active:scale-[1.01] active:-translate-y-0.5",
            theme === 'dark' ? "bg-[#161616]" : "bg-black/[0.02] hover:bg-white active:bg-white",
            brandBorder(theme, isActive)
        )}>
            ...
            <div className="flex items-start justify-between relative z-10">
                ...
                <div className="p-1.5 rounded-xl">
                    <Icon className={cn(
                        "w-4 h-4 mt-0.5 flex-shrink-0 transition-all duration-300 group-hover:scale-110 group-hover:text-brand group-active:scale-110 group-active:text-brand group-focus-within:text-brand",
...
function SidebarLink({ href, icon: Icon, title, desc, theme, pathname, onClick, external }: any) {
    ...
    const className = cn(
        'group flex items-center gap-4 rounded-2xl border p-4 transition-all duration-500 overflow-hidden relative cursor-pointer text-left w-full touch-manipulation',
        'hover:scale-[1.02] hover:-translate-x-1 active:scale-[1.01] active:-translate-x-0.5',
        theme === 'dark' ? 'bg-[#161616]' : 'bg-black/[0.02] hover:bg-white active:bg-white',
        brandBorder(theme, isActive)
    );
    ...
    const content = (
        <>
            ...
            <Icon className={cn(
                "w-5 h-5 relative z-10 transition-all duration-300 group-hover:scale-110 group-hover:text-brand group-active:scale-110 group-active:text-brand group-focus-within:text-brand",
                theme === 'dark' ? "text-white/45" : "text-black/45",
                isActive && "text-brand"
            )} />
        </>
...
export default function CardNav({
    ...
    const fullAboutNav = SITE_FEATURES.fullAboutNav;
    const allHrefs = fullAboutNav
        ? ['/projects', '/experience', '/skills', '/blog', '/gallery', '/resume']
        ...
        <div ref={containerRef} className="relative flex items-center h-10">
            ...
            <AnimatePresence>
                {isExpanded && (
                    ...
                    >
                        <div className={cn(
                            "relative w-[min(920px,calc(100vw-2rem))] max-w-[calc(100vw-2rem)] max-h-[min(80vh,720px)] overflow-y-auto rounded-[1.5rem] border shadow-2xl flex flex-col lg:flex-row lg:items-stretch backdrop-blur-2xl transition-all",
                            theme === 'dark'
                            ...
                            <div className="flex-1 p-4 md:p-5 flex flex-col gap-3 md:gap-4 min-w-0">
                                <div
                                    className={cn(
                                        ...
                                        !fullAboutNav && 'flex-1 h-full min-h-0 md:min-h-[18rem] lg:min-h-0'
                                    )}
                                >
                                    <MegaBoxBig
                                        href="/projects"
                                ...
                                {/* Restored layer: Skills / Linktree / Blog row */}
                                {fullAboutNav && (
                            ...
                            {/* Right Sidebar */}
                            <div className={cn(
                                "w-full lg:w-[260px] lg:shrink-0 p-4 flex flex-col justify-center gap-3 md:gap-4 border-t lg:border-t-0 lg:border-l",
