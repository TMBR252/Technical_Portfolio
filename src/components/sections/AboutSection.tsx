...
const formatPeriod = (startDate: string, endDate?: string, isOngoing?: boolean) => {
...
const showcaseMembers = [
    ...portfolioData.experiences.slice(0, 4).map((exp) => ({
        id: exp.id,
        name: exp.company,
        role: exp.position.replace(' (Contract-Based)', ''),
        description: exp.description,
        period: formatPeriod(exp.startDate, exp.endDate, exp.isOngoing),
        image: 'https://images.unsplash.com/photo-1497366216548-37526070297c?q=80&w=1964&auto=format&fit=crop',
                        ...
                        <div className="md:col-span-7 flex flex-col sm:flex-row gap-8 text-[13px]">
                            ...
                            <div className="flex-1 space-y-3 flex flex-col">
                                ...
                                <div className="mt-6 md:mt-auto pt-4">
                                    <span className="text-3xl lg:text-4xl font-signature text-zinc-900 dark:text-white/90">{t('leadIn.signature', { name: portfolioData.personal.name })}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
const ScrollHijackSection = () => {
    ...
    // Start progress as the section enters the viewport (while hero is still leaving),
    // so the card rise animates over the hero instead of after a blank black stretch.
    const { scrollYProgress } = useScroll({
        target: sectionRef,
    ...
    // Mobile: stack chapters vertically - no horizontal hijack
    // CoreEngineeringPanel (Professional Statistics) suppressed
    if (isMobile) {
        return (
            <div className="relative w-full overflow-x-hidden">
                <div className="min-h-[100dvh] w-full">
                    <IdentitySequence isVisible scrollYProgress={smoothProgress} />
                </div>
            </div>
        );
    }

    // Stats / lead-in suppressed - IdentitySequence only (entrance animations intact)
    return (
        <div ref={sectionRef} className="relative h-[400vh]">
            <div className="sticky top-0 h-[100dvh] w-full overflow-hidden z-10">
                ...
                >
                    {/* Top decorative curved border suppressed */}
...
export default function AboutSection() {
    ...
    return (
        ...
        >
            {/* AboutLeadIn (Core Focus) sticky plane suppressed */}

            {/* Overlay Layer - Hijack Zone & Footer */}
                                ...
                                <HorizontalTimeline data={showcaseMembers.map((member) => ({
                                    ...
                                    content: member.id === 'view-more' ? (
                                        <Link
                                            ...
                                            className="relative flex items-center h-[140px] w-full max-w-[250px] z-30"
                                        >
                                        ...
                                        <div className="flex flex-col gap-4 w-full max-w-[320px] md:max-w-[400px] border border-neutral-200 dark:border-neutral-800 p-6 rounded-2xl bg-white/80 dark:bg-neutral-950/80 backdrop-blur-md shadow-xl mt-4">
