...
export const IdentitySequence = ({ scrollYProgress, isVisible }: IdentitySequenceProps) => {
    ...
    // Map parent scroll to local progress. Entrance animations restored;
    // parent scroll starts earlier (offset) so the rise plays over the hero exit.
    const localProgress = useTransform(scrollYProgress, [0, 0.75], [0, 1]);
    ...
    const marqueeItems = [
        ...
        <div key="icon" className="w-32 h-32 md:w-48 md:h-48 rounded-full bg-brand flex items-center justify-center mx-12">
    ...
    return (
        <div className="relative w-screen h-full flex flex-col items-center justify-center overflow-hidden bg-background dark:bg-black">
            {/* Phase 0: Lead-in UI - About Me CTA + Scroll to Explore suppressed; keep short story label */}
            <motion.div
                style={{ opacity: phase0Opacity }}
                className="absolute inset-0 z-0 flex flex-col items-end justify-end pointer-events-none pb-16 md:pb-20 px-12"
            >
