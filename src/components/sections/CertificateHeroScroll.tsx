...
const CertificateHeroScroll: FC<CertificateHeroScrollProps> = ({ onDownloadClick, isLowPowerMode: isLowPowerModeProp }) => {
    ...
    const getPositions = useCallback((): Positions => {
        ...
        const isCurrentlyMobile = vw < 768; // Tailwind md
    ...
    return (
        <>
            ...
            <div ref={spacerRef} className="h-[220vh] md:h-[350vh] w-full relative z-10 pointer-events-none" />

            {/* FIXED HERO: Stays behind content. z-10 ensures it sits ABOVE particles (z-0), but transparent to see them. */}
            <div ref={fixedContainerRef} className="fixed inset-0 z-10 h-[100dvh] w-full overflow-hidden bg-transparent pointer-events-none">
                {/* Background Effects */}
